...
async function activate(letta) {
  ...
  const trace = (m) => {
    try {
      ...
      globalThis.__autopivotFs.appendFileSync(f, `[${(/* @__PURE__ */ new Date()).toISOString()}] ${m}
`);
    } catch {
    }
  };
  ...
  try {
    ...
    log(`capabilities unreadable: ${e?.message ?? e}`);
  }
  const llmEvents = letta.capabilities?.events?.llm;
  if (llmEvents === false) {
    log("DEGRADED: this host does not emit llm_start/llm_end (capabilities.events.llm=false). Completion-failure detection \u2014 rate limits, auth failures, stalls \u2014 CANNOT run here. The reachability ladder still works; failover on a failed completion does not.");
  }
  ...
  for (const [name, handler] of [
    ...
    ["llm_end", (event, ctx) => {
      ...
      log(`llm_end model=${model} agent=${agentKey ?? "-"} failed=${failed} stopReason=${event?.stopReason ?? "-"} reason=${JSON.stringify(reason)?.slice(0, 160)}`);
      if (failed) {
        try {
