...
/**
 * Normalize the optional `agents` map: agent id OR agent name -> override.
 *
 * WHY THIS EXISTS: `primary` is a single global handle, and it wins for every NEW
 * conversation regardless of what an agent's own record says. On a fleet that is
 * a real problem, not a nicety — verified 2026-09-02, the docs agent (configured
 * deepseek-v4-flash, chosen for cost) was running the ladder's primary instead,
 * as were four other utility agents. One global model for a fleet with per-agent
 * cost and capability needs cannot express "pulse runs cheap, Kinara runs strong".
 *
 * SCOPE: an override may set `primary` (and optional `contextWindow` /
 * `reasoningEffort` carried onto the switch). The RUNGS remain global — the probe
 * and suspension machinery is built once per process at activate time, and a
 * per-agent ladder would mean rebuilding probes per agent. Fallback behaviour is
 * therefore shared; only the top of the ladder is per-agent. That covers the
 * observed need and leaves the harder change unbuilt until something asks for it.
 */
function normalizeAgents(raw, warnings) {
  ...
  for (const [key, value] of Object.entries(raw)) {
    ...
    // Optional PER-AGENT rungs. Absent means "share the global ladder", which is
    // what every existing config expresses, so this stays backwards compatible.
    // Present means this agent's fallbacks are its own — a fleet where Kinara
    // falls back to a paid frontier model while pulse falls back to something
    // cheap cannot be said with one shared list.
    let rules;
...
/**
 * Effective primary for an agent. Identity is matched by id FIRST, then by name —
 * ids are stable, names are what a human recognizes, and supporting both means the
 * config stays readable without becoming ambiguous. Returns the global primary when
 * no override matches, so an unlisted agent behaves exactly as before.
 */
export function agentPrimary(config, identity = {}) {
...
export function defaultConfig() {
  return {
    ...
    agents: {},    // optional per-agent primary overrides (see normalizeAgents)
    rules: [],
