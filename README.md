...
```bash
letta install npm:letta-mod-autopivot
```

Or straight from this repo:

```bash
letta install git:github.com/chaddorsey/letta-mod-autopivot
```
...
```bash
npm install        # dev deps (esbuild, @inquirer/prompts)
npm test           # node --test — 146 zero-dependency unit tests
npm run build      # bundles dist/autopivot.mjs (mod) + dist/autopivot-configure.cjs (configurator)
```

The mod entry loaded by Letta is the bundled `dist/autopivot.mjs`; the readable source lives in `index.mjs` + `lib/`. See [`docs/LADDER-DESIGN.md`](./docs/LADDER-DESIGN.md) for the design and the empirically verified detection contract behind the stall watchdog.
